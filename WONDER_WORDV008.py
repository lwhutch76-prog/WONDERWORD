#----------------------------VERSION 8------AUG 6 2026



#Ehancements

#change SCREEN_WIDTH from 1000 to 1100 or 1200.......advan vs disadvan?



# NEED TO REMOVE ALL DEGUG STATEMENTS FOR VERSION 5.....
...
if platform.system() == "Darwin":
    # On macOS: Redirect writing to the user's visible Documents folder to guarantee permissions
    mac_save_dir = Path.home() / "Documents" / "WonderWord"
    mac_save_dir.mkdir(parents=True, exist_ok=True)
    score_file_path = mac_save_dir / "HIGHSCORES.txt"
    
    # One-time deployment: Clone your default high scores list if it's a fresh installation
    if not score_file_path.exists() and master_score_path.exists():
        try:
            shutil.copy(master_score_path, score_file_path)
        except Exception:
            pass
            
    # --- COMMENTED OUT FALLBACK SAFETY NET (For after your call) ---
    if not score_file_path.exists():
        try:
            with open(score_file_path, "w") as f:
                f.write("100000,Player 1,5,0.5,July 16 2026 12:00PM\n")
        except Exception:
            pass
    # -----------------------------------------------------------------
else:
    # On Windows: Retain your original executable directory logic
...
try:
    ...
    base_snapshot = pygame.image.load(game_board_path).convert()
    
    # Check the size and auto-scale if it is wrong
...
try:
    ...
    sound_happy_trails.set_volume(0.3)
...
pygame.display.set_caption("WONDER WORD Version 8")
...
#Load the image from the same folder
base_snapshot = pygame.image.load("GAME_BOARD.png").convert()
#Check the size and auto-scale if it is wrong
if base_snapshot.get_width() != 1000 or base_snapshot.get_height() != 600:
    base_snapshot = pygame.transform.scale(base_snapshot, (1000, 600))
...
def draw_origin_story_screen():
    ...
    story_paragraphs = [
        ...
        "was deployed as a logical weapon during that wartime mail exchange. The ",
        "math required to decipher it was so brutally complex and frustrating that",
        "the Mathematician promptly quit the match in protest!",
        divider_line,
...
HELP_DATA = {
    ...
    2: {
        ...
        "text_y":  50,
        "lines": [
            "You start each game with a bank of 200,000 points.",
...
def save_high_score(name, final_score, total_guesses, seconds):
    ...
    try:
        ...
        # Catches Mac PermissionErrors or Windows Read-Only locks gracefully
    ...
    try:
        with open(score_file_path, "r", encoding="utf-8") as f:
            for line in f:
                parts = line.strip().split(",")
                if len(parts) == 5:
                    # Explicit array mapping to read historical score data properly
                    scores.append((int(parts[0]), parts[1], parts[2], parts[3], parts[4]))
...
def quit_game():
    ...
    rect_word = text_word.get_rect(center=(150, 480))
    rect_score = text_score.get_rect(center=(150, 520))
    rect_thanks = text_thanks.get_rect(center=(150, 560))
    ...
    # How long the song plays at full volume before fading (in milliseconds)
    #play_time_ms = 11000  
...
#confirm the placement of tis dead code.................VISUAL STUDIO CODE to compare.......

#also fix get hub based on google ai instruction for opera
            
#look for more sounds
            

# change the sound volmes for the fireworks........look at google AI save from last night
...
#                # STATE 2: TIMEOUT RESUMPTION PROCESSING
#                if game_state == "TIMEOUT":
#                    break_end = datetime.datetime.now() # Fixed truncated break assignment             
