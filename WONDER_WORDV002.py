...
# REDO GAME BOARD - Snapshot board......convert to 1000 x 600 ...confirm it fits
# HELP screens.........then go to increase quality website
...
# limit splash screen name so it wont fuck up leader board
# when in help screen.........if a letter is clicked have to hit ENTER twice
# A lot of Y/N questions...........any letter but Y will take N logic....check Y/N only
# no keyboard enter to turn letter back to black
# change name of Roy_Rogers png
# REDO SCOREBOAR.PNG FOR HELP .....SCRATCHPAD MISSPLELED
...
#help with buffer lag in clicking for typing 
...
if platform.system() == "Darwin":
    # On macOS: Use the Shared folder to permanently bypass App Translocation
    mac_save_dir = Path("/Users/Shared/WonderWord")
    mac_save_dir.mkdir(parents=True, exist_ok=True)
    score_file_path = mac_save_dir / "HIGHSCORES.txt"
else:
    # On Windows/Linux: Retain your original logic
    score_file_path = exe_dir / "HIGHSCORES.txt"

# UNIFIED DEPLOYMENT: Initialize file if it doesn't exist yet
if not score_file_path.exists():
    copied_successfully = False
    
    # Strategy A: Try to copy the funny names directly from the built-in bundle
    if master_score_path.exists():
        try:
            with open(master_score_path, "r", encoding="utf-8") as source:
                content = source.read()
            with open(score_file_path, "w", encoding="utf-8") as target:
                target.write(content)
            copied_successfully = True
        except Exception:
            copied_successfully = False
            
    # Strategy B: Hardcoded 9-name fallback if the bundle file can't be read
    #if not copied_successfully:
        #try:
            #with open(score_file_path, "w", encoding="utf-8") as f:
                #f.write("399788,TONY,1,0.1,08/21/26  06:23PM\n") 
                #f.write("383156,WAYNE,7,1.9,08/04/26  01:27PM\n") 
                #f.write("299788,DANNY,1,0.1,08/18/26  03:21PM\n") 
                #f.write("287632,HOLLAND,3,0.8,08/16/26  09:17AM\n") 	
                #f.write("271778,FRANK,5,1.8,08/21/26  11:19AM\n") 
                #f.write("260288,MARK,8,4.7,08/11/26  05:33PM\n") 
                #f.write("259784,D TRUMP,3,0.1,08/19/26  04:07PM\n") 
                #f.write("243512,ROBERT,7,0.9,08/12/26  11:07PM\n")
                #f.write("239352,JACKIE,7,0.9,08/19/26  1:07AM\n")
        #except Exception:
            #pass
    ...
    if not copied_successfully:
        try:
            with open(score_file_path, "w", encoding="utf-8") as f:
                ...
                f.write("399788,MARIA,1,0.1,07/28/26  06:23PM\n") 
                f.write("383156,ELAINE,7,1.9,07/04/26  01:27PM\n") 
                f.write("299788,JAN,1,0.1,07/28/26  03:21PM\n") 
                f.write("187632,HOLLY,3,0.8,07/16/26  09:17AM\n") 	
                f.write("271778,TRACI,5,1.8,07/21/26  11:19AM\n") 
                f.write("260288,GINA,8,4.7,07/31/26  05:33PM\n") 
                f.write("159784,D TRUMP,3,0.1,07/19/26  04:07PM\n") 
                f.write("243512,STEVEN,7,0.9,08/12/26  11:07PM\n")
...
HELP_DATA = {
    ...
    4: {
        ...
        "lines": [
           ...
           "",
           "After 10 guesses, the game will begin to make 'SUGGESTED GUESSES' to assit you.",
           "",
           "                       PRESS ENTER TO RETURN TO INDEX"
        ]
    },
...
def display_score(): 
    ...
    if number_of_guesses > 10:
     ...
     # UPDATED: Cleaned up the variable tracking. We no longer call the function here.
        # Instead, we read directly from your permanent suggested_hints list, which stops the blinking!
        if suggested_hints:
...
def clear_high_scores():
    ...
    if platform.system() == "Darwin":
        score_file_path = Path("/Users/Shared/WonderWord/HIGHSCORES.txt")
    else:
        global exe_dir
        score_file_path = exe_dir / "HIGHSCORES.txt"
...
def draw_high_scores():
    # FORCE the function to look at the Shared folder directly to beat the Mac Sandbox
    if platform.system() == "Darwin":
        score_file_path = Path("/Users/Shared/WonderWord/HIGHSCORES.txt")
    else:
        global exe_dir
    ...
    try:
        ...
        current_y = 185
        for i, (s, n, g, t, dt) in enumerate(scores[:9], 1):                                    
...
def save_high_score(name, final_score, total_guesses, seconds):
    ...
    print(f"[SANDBOX CHECK] The game is writing to this exact path: {score_file_path.absolute()}")
    ...
    # Step 1: Read existing scores into memory first if the file exists
    if score_file_path.exists():
        try:
            ...
            if 'mac_save_dir' in globals():
                print(f"[STARTUP CHECK] READ Mac Save Directory variable is set to: {mac_save_dir.absolute()}")
            #print(f"[STARTUP CHECK] READ Does the highscore file exist at that path?: {score_file_path.exists()}")
            #print(f"[DEBUG] Failed to read leaderboard before saving: {e}")
            #print("\n[CRITICAL ERROR - READ STEP FAILED]")
            #print(f"Error Message: {e}")
            #print(f"Path Function Attempted to Read: {score_file_path}")
            #print(f"Does that file actually exist right now?: {score_file_path.exists()}")
            #print(f"Number of scores successfully recovered into memory: {len(scores)}")

    # Step 2: Add the current player's new score into the memory list
    scores.append((int(final_score), safe_name, str(total_guesses), str(seconds), timestamp))
    
    # Step 3: Sort the list in memory (highest score first)
    scores.sort(key=lambda x: x[0], reverse=True)
    
    # Step 4: Write exactly the top 9 scores back to the file in one shot
    try:
        with open(score_file_path, "w", encoding="utf-8") as f:
        ...
        if 'mac_save_dir' in globals():
            print(f"[STARTUP CHECK] WRITE Mac Save Directory variable is set to: {mac_save_dir.absolute()}")
        #print(f"[STARTUP CHECK] WRITE Does the highscore file exist at that path?: {score_file_path.exists()}")
        #print(f"[PERMISSION ALERT] File write blocked by OS: {e}")    
        #print("\n[CRITICAL ERROR - WRITE STEP FAILED]")
        #print(f"Error Message: {e}")
        #print(f"Path Function Attempted to Write: {score_file_path}")
        #print(f"How many names were in the list when trying to save?: {len(scores)}")    
...
game_deck = word_list.copy()    # NEW LINE: Makes a copy of the list
random.shuffle(game_deck)       # NEW LINE: Shuffles the words like cards
hold_secret_word = game_deck.pop() # NEW LINE: Takes the first word off the pile



# --- SECOND FUNCTION (The new one goes right here!) ---
def get_next_secret_word():
    global game_deck
    if not game_deck:
        game_deck = word_list.copy()
...
def get_strategic_hints(word_bank, letters_not_in_word, letters_in_word, letters_might_be_in_word, hold_secret_word, already_suggested, top_n=2):
    """
    ...
    for word in word_bank:
        ...
        # EXTRA ROADBLOCK: Skip words that have already been shown to the player
        if word_upper in already_suggested:
            continue
    ...
    # Sort strictly by the score (index 1 of the tuple), from highest to lowest
    scored_words.sort(key=lambda x: x[1], reverse=True)
    
    # Filter out words that give 0 new elimination value (keeps just the word strings)
    ...
    if best_fillers and len(scored_words) > 0:
        # 1. Grab th > 0e absolute maximum score value from the first item
        highest_score = scored_words[0][1]
        
        # 2. UNPACK CORRECTLY: Extract the word string out of the tuple if its score matches
        top_tier_pool = [word for word, score in scored_words if score == highest_score and word not in already_suggested]
        
        # 3. FALLBACK: If all top-tier words are used up, grab the next best clean fillers
        if not top_tier_pool:
            top_tier_pool = [word for word in best_fillers if word not in already_suggested][:15]
            
        # 4. Shuffle the deck to destroy alphabetical bias
        random.shuffle(top_tier_pool)
        
        # Randomly select your words from this freshly mixed pool
        return random.sample(top_tier_pool, min(top_n, len(top_tier_pool)))
        
    # FALLBACK: If no fillers remain, shuffle the whole legal deck to break alphabetical order!
    if possible_solutions:
        shuffled_solutions = list(possible_solutions)
        random.shuffle(shuffled_solutions)
        
        # Now we grab the first few words from our freshly mixed deck
        pool_size = min(10, len(shuffled_solutions))
        top_tier_solutions = shuffled_solutions[:pool_size]
...
def quit_game():
    ...
    TEXT_MAIN = (248, 248, 255) 

    # --- SCALE THE CANVAS FIRST ---
    # We blow up the background snapshot to the full window size first
    scaled_canvas = pygame.transform.scale(canvas, (current_width, current_height))

    # --- RENDER TEXT ---
    # Pygame renders anti-aliased text beautifully when drawn directly at high resolutions
    # Note: Removed the extra surface-copying layer as it causes pixel artifacting with transparency
    text_word = font_sz_16x.render(f"The Wonder Word was: {hold_secret_word}", True, TEXT_MAIN)
    text_score = font_sz_16x.render(f"Your Final Score Was: {points}", True, TEXT_MAIN)
    text_thanks = font_sz_16x.render(f"THANKS FOR PLAYING - {active_name}", True, TEXT_MAIN)

    # --- CENTER ON THE ACTUAL WINDOW SIZE ---
    # Instead of hardcoded (150, 20), we center it dynamically on the actual screen width/height
    #center_x = current_width // 2
    #center_y = current_height // 2
 
      
     # 2. Position the text precisely in your top-left corner zone
    rect_word = text_word.get_rect(topleft=(3, 20))
    rect_score = text_score.get_rect(topleft=(3, 50))
    rect_thanks = text_thanks.get_rect(topleft=(3, 80))
    ...
    #rect_word = text_word.get_rect(center=(center_x, center_y - 40))
    #rect_score = text_score.get_rect(center=(center_x, center_y))
    #rect_thanks = text_thanks.get_rect(center=(center_x, center_y + 40))
    
    # --- DRAW TEXT DIRECTLY TO THE SCALED CANVAS ---
    scaled_canvas.blit(text_word, rect_word) 
    scaled_canvas.blit(text_score, rect_score)
    scaled_canvas.blit(text_thanks, rect_thanks)
    
    # Flip the finished, crisp screen to the user
    SCREEN.blit(scaled_canvas, (0, 0))
    pygame.display.flip() 
    
    # ========================================================
    # TIMING AND FADE CONTROLS
    # ========================================================
    play_time_ms = 9999  
    fade_time_ms = 1500   
...
#HOWEVER DELETING THIS CODE STOPS THE HELP SCREENS FROM WORKING AUG 10 2026
            ...
            if event.key == pygame.K_RETURN:
                ...
                # --- STRATEGIC HINT ENGINE TRIGGER ---
                # This calculates the hints EXACTLY ONCE per turn when Enter is hit!
                if number_of_guesses > 10:
                    strategic_tips = get_strategic_hints(
                        large_hint_bank, 
                        letters_not_in_word, 
                        letters_in_word, 
                        letters_might_be_in_word, 
                        hold_secret_word,
                        suggested_hints # Pass your running list to serve as the function's memory
                    )
                    
                    if strategic_tips:
                        suggested_hints = strategic_tips # Freeze the new words for the screen to draw
...
#insert new code end
                ...
                if game_state == "GAME_OVER":
                    ...
                    if processed_input == "Y":
                        ...
                        # -------------------------------------------------------------
                        # CHANGE TO THESE 4 LINES:
                        # -------------------------------------------------------------
                        if len(game_deck) == 0:        # If we run out of words
                            game_deck = word_list.copy() # Get a fresh list
                            random.shuffle(game_deck)    # Shuffle them again
                        hold_secret_word = game_deck.pop() # Pull the next unique word
                        # -------------------------------------------------------------
...
# 5-Letter Word Entry Evaluations
                    ...
                    # [REMOCKED HINTS FROM HERE TO MOVE THEM TO THE BOTTOM]
...
#/insert code start
     # --- FIXED LOCATION: CALLED AFTER GUESS IS REGISTERED ---
                    if number_of_guesses > 10:
                        strategic_tips = get_strategic_hints(
                            large_hint_bank, 
                            letters_not_in_word, 
                        ...
                        if strategic_tips:
                            # FIX: Use suggested_hints so display_score() can find it immediately!
                            suggested_hints = strategic_tips
                            
                            # 2. Append the new words to your master memory block so they are forever banned!
                            for word in strategic_tips:
                                if word not in already_suggested:
                                    already_suggested.append(word)                  
