...
if platform.system() == "Darwin":
    # On macOS: Use the dedicated App Support zone
...
# UNIFIED DEPLOYMENT: Initialize file using memory streams to beat the Mac Sandbox
if not score_file_path.exists():
    copied_successfully = False
    
    # Strategy A: Safely stream text out of the bundle to prevent OS copy blocks
    ...
    # Strategy B: Complete 9-name hardcoded fallback if bundle reading fails
    if not copied_successfully:
        try:
            with open(score_file_path, "w", encoding="utf-8") as f:
                ...
                f.write("2393512,BILLIE,7,0.9,08/12/26  1:07AM\n")
